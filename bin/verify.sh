...
# exists (WT="" is a cleanup no-op). INT/TERM run cleanup and then `exit` explicitly with the signal's own
# code (130/143): without that explicit exit, bash does NOT terminate after a custom trap for a non-EXIT
# signal — it resumes the script right where the signal landed. That is exactly how a killed run used to keep
# going: cleanup already `cd`s back to $ROOT, so whatever ran next (lint, type-check, the test command) ran
# there, against the main checkout, and the script still reached its own "verify: GREEN" at the end, exit 0,
# unconnected to anything it actually checked.
WT=""
...
trap cleanup EXIT
on_signal(){ trap - EXIT INT TERM; cleanup; exit "$1"; }
