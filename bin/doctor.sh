...
# 2d. board index drift (PI-62): tasks/INDEX.md is generated by tasks-index.sh and every lane is
# supposed to re-run it after a merge; nothing enforced that until now, so a stale row (a task marked
# Done on disk that the index still shows as Todo, with no PR link) reached bin/status.sh and made the
# next session open on a board state that disagreed with the disk it claims to trust. Read-only: this
# check never writes tasks/INDEX.md itself, it only names what is stale and prints the one command
# that regenerates it. Skips in silence when there is no tasks/ directory at all (AC5), when the board
# has no committed tasks/INDEX.md yet (nothing to be stale against — a project that has never run
# tasks-index.sh is a different gap, not this one), or when tasks-index.sh itself cannot be found or
# fails to run — the same house style as every other check in this section that depends on an input
# doctor cannot assume is there.
if os.path.isdir("tasks"):
    tasks_index_abs = os.path.join(os.path.dirname(doctor_abs), "tasks-index.sh")
    index_path = "tasks/INDEX.md"
    if os.path.exists(tasks_index_abs) and os.path.exists(index_path):
        try:
            _gen = subprocess.run(["bash", tasks_index_abs, "--stdout"], capture_output=True, text=True, timeout=10)
        except Exception:
            _gen = None
        if _gen is not None and _gen.returncode == 0:
            generated = _gen.stdout
            fix_cmd = f"bash {shlex.quote(tasks_index_abs)}"
            current = open(index_path, errors="ignore").read()
            if generated.strip() != current.strip():
                def _index_rows(text):
                    # keyed by the ID cell; each value is the tuple of trimmed cells, so pipe
                    # spacing added or removed by a markdown-reformatting commit hook never counts
                    # as drift — only a changed cell does (see the module note above)
                    out = {}
                    for line in text.splitlines():
                        s = line.strip()
                        if not s.startswith("|"): continue
                        cells = [c.strip() for c in s.strip("|").split("|")]
                        if not cells or not cells[0] or re.match(r"^-+$", cells[0]) or cells[0] == "ID": continue
                        out[cells[0]] = tuple(cells)
                    return out
                gen_rows, cur_rows = _index_rows(generated), _index_rows(current)
                diff_ids = sorted(
                    (set(gen_rows) ^ set(cur_rows))
                    | {tid for tid in gen_rows.keys() & cur_rows.keys() if gen_rows[tid] != cur_rows[tid]}
                )
                if diff_ids:
                    err(f"tasks/INDEX.md is stale — differs from tasks-index.sh for: {', '.join(diff_ids)} — regenerate it: {fix_cmd}")
                else:
                    err(f"tasks/INDEX.md differs from tasks-index.sh output but no task row differs (formatting-only difference) — regenerate it: {fix_cmd}")
