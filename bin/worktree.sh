...
#   Worktree lands at <repo>/.claude/worktrees/<branch-with-slashes-replaced-by-dashes>. If origin/<branch>
#   already exists it is checked out tracking it; otherwise a new branch is created off origin/<base> (base
#   defaults to .pocket-it.json's baseBranch, else main) after a git fetch. A second run for the same branch
#   reuses the existing worktree (idempotent). Adds ".claude/worktrees/" to <repo>/.git/info/exclude once, so
#   the directory never shows as untracked in the main checkout.
