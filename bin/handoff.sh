...
# Creates docs/SESSION_HANDOFF.md if missing. Log keeps the last LOGCAP lines; the rest are moved (never dropped)
# to docs/SESSION_HANDOFF_ARCHIVE.md, oldest entry at the top throughout the whole archive — within a batch and
# across rotations alike, one single order, not two. Facts are capped at CAP (see below) — at the cap a new
# fact is refused (exit 3), it does not drop the oldest.
set -uo pipefail
CAP=100
LOGCAP=40
