...
Do not fail, and exclude **only the axis being intentionally truncated, never the whole element** — the compass procedure is written for unintended clipping, not for a technique that truncates on purpose using the same properties it neutralises: text hidden on purpose for sighted users (`sr-only`/visually-hidden patterns, `clip: rect(0,0,0,0)`, 1×1px) is excluded entirely; scrollable containers (`overflow: auto`/`scroll`, where the rest of the content is reachable by scrolling, not clipped) are excluded entirely; `text-overflow: ellipsis` and a marquee/ticker (compass #8) truncate **horizontally only** — run the procedure without neutralising the property performing that truncation, and only for that axis; the vertical axis of the same element still gets the full procedure (a one-line ellipsis title with a tight `line-height` still fails if its descenders are cut, and the marquee track itself must not clip vertically); `-webkit-line-clamp` truncates **below its last line only** — the sides of every kept line are still checked in full. The exclusion applies to the specific ancestor doing the intentional truncation, not to every ancestor in the chain: an undersized wrapper around an ellipsis title is a real vertical defect even though the title inside it also truncates horizontally on purpose.
