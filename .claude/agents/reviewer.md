...
Line 1: CI mode (`none` / `dev` / `prod`), `verify.sh` result, and your own `Effort: {N} min elapsed — agent-{id}` in the one unit every report uses (shared rules §7; `Effort: not available — no start timestamp` if you did not take it, never seconds or turns in its place). Then one line per PR: `{ID} — PR #{N} — APPROVED (merge: orchestrator|user) | NEEDS WORK: {n} findings, first: {six words} — cause: {first-round|example-not-class|base-moved|verification-reintroduced|other: …} — fix at: {n/a|task file|developer prompt|shared/implementing-common.md|…}`. One line for conflicts resolved / CI fixes dispatched, if any.
