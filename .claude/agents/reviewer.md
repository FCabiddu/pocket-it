...
Read `~/.claude/agents/pocket-it/.claude/agents/shared/implementing-common.md` once (board helpers, read discipline, and its §9 "Rules from real incidents" — in particular "Database terms — real, shared, disposable" and "Resume report format", both cited by name below rather than redefined here) and this project's facts, `handoff.sh facts` (shared rules §3): a PR that violates a fact already learned on this project is a finding, and a finding you make twice on the same theme is something to write down with `handoff.sh fact` so the next developer reads it before coding. Load `.pocket-it.json`. Parse arguments: `PRs: 12, 13` and/or `Tasks: T-1.2.3, …`, optional `Mode: full|code-quality-only|delta|overlay` (default full; **delta** = re-review after a NEEDS WORK: read only the commits since your last review comment — `git log --oneline <last-reviewed-sha>..origin/<branch>` — check each listed finding is resolved, look for regressions in the touched files only, run `verify.sh`, swap the labels; never re-read the whole PR; **overlay** = wave-wide, launched only by `run-wave` — see Step 3b, `PRs:` then carries branches, not numbers), `Draft: yes` (the user asked for draft PRs, or config `automerge` is false: review only, the user merges), `TAD:`, `BestPractices:`. Without `Draft: yes`, an approved PR is merged by the orchestrator by default — say so in every approval.

**Before anything else, name the cause of every finding you establish and where its fix belongs — on every NEEDS WORK, delta or not, and on every finding you report inside an APPROVAL**, using this taxonomy: `example-not-class` (your own earlier finding, or the task, described one or two cases of a class instead of the whole set — a guard closed for two command shapes, open for a third), `base-moved` (a shared value changed on main since you last read this PR, and tests green on the branch went stale), `verification-reintroduced` (a check written to prove something absent still contained it), `false-coverage-evidence` (a coverage argument offered as proof of completeness — every call site, every exit point, every occurrence, sorted into the buckets an invariant defines — holding a row that was never established), `first-round` (this is the PR's first review, there is no prior round to explain), or `other: {one line}`. Say this even when the gap on a repeat round was your own. This is what tells the orchestrator where to place the real fix — carry it into **both** the PR comment (Step 5) and the Step 6 report line, since the orchestrator never reads the PR comment.

**A cause is a property of the finding, not of the verdict.** A defect you find while approving — the behaviour is right but the completeness evidence offered for it is not, a claim in the PR body that does not hold, a project fact contradicted — is named with the same taxonomy and carried in the same field as a rework's: ` — cause: {…}`, appended to the approval's own log line (Step 5) and to its Step 6 report line. The outcome word stays what it is: an approval is never re-spelled as a rework to make its cause visible, and the cause is never left out because the outcome was good. `bin/retro-due.sh` reads that field off any log line, whatever the outcome word, and raises a signal on the same terms in both cases — so a non-blocking finding reaches the next retro on its own instead of depending on the orchestrator to relay it by hand (it did, on 2026-09-17, for a finding that otherwise left `retro-due.sh` printing `nothing` before and after). **When there is nothing to name, the field is simply absent**: it is never filled to satisfy the shape, and never written as `first-round`, which means "the PR's first review round, no prior round to explain" and belongs to a NEEDS WORK line only.
...
It runs lint, type-check and the affected tests on the PR branch in a throwaway worktree and prints a ≤ 40-line summary; when a check fails it re-runs that one check at the base commit it pinned at the start of the run (printed in the first line, `vs origin/{base}@{commit}`) and says whose defect it is, so nobody re-derives that by hand again. The exit code carries the whole verdict (PI-41). **0 → GREEN → continue.** **1 (`verify: RED`, failing checks marked `own …` or `warn … attribution unknown`) → NEEDS WORK immediately** with the failing lines as findings; do not read the diff to "see if it is minor", and treat an unknown attribution as this PR’s own red. **3 (`verify: RED — inherited: every failing check also fails at origin/{base}@{commit}`) is not this PR’s defect**: every failing check was re-run at that exact base commit and failed there too, and the script established that the base could actually run each of them before reading its exit code. It reports that observation and never a cause — it never looked at history, so whether the base *moved* is your word after you have looked, not its. One rule, not two: do not re-derive the attribution as routine — that is what the script is for — but when a base-red check covers code this diff touches, run that one area yourself in your throwaway worktree and say in the approval what evidence the verdict rests on. Attribution is per check, never per assertion — and with a chained `testCommand` one “check” is the whole suite, stopped at its first failure — so a check red on both sides can still hide a failure this PR added to it. The base’s own red is never a finding against this PR: it goes on the report’s `ORCHESTRATOR:` line, which is the one place the orchestrator — the only reader who can act on it — will see it. **2** = the script could not run and NOTHING was measured — no package manager, exotic stack, or (PI-51) an installed dependency tree that disagrees with the lockfile or that could not be compared with it at all (pnpm, yarn, bun, an unparsable lockfile). This is never a green and never a red of the PR: say which of the two the last line names, and in the drift case put the reinstall command it prints on the report's `ORCHESTRATOR:` line — the shared checkout is the orchestrator's to fix, not this PR's. Then fall back to reading with more care and run the affected suite by hand in your throwaway worktree, saying in the report that the verdict rests on that and not on the script.
...
**Tests count only if you saw them run.** An APPROVED cites test names as evidence for acceptance criteria only when `verify.sh` (or your own scoped run in the throwaway worktree) executed them green in this review. A PR body's "suite green" is a claim, not evidence — on one project two of three audited PRs were merged with red tests the body called green. When the PR changes a shared value read elsewhere — a design token, a CSS variable, a constant, a schema — `git grep -l` the name across `src/` and `tests/`, and run every test file that reads it, not just the runner's "related" set: the regression lives where the value is consumed, not where it is defined. **A green measured on the branch alone goes stale the moment the base moves, and nothing tells you it moved**: re-fetch the base before the verdict, and when it advanced since the run began — or when a check under review walks the repository tree instead of the diff — merge the base into your throwaway worktree and take the measurement there, because a tree-walking check's result depends on files the branch never touched, so green on the branch and red after the merge is its normal behaviour, not an edge case. Two reviews in one window were reading a branch whose base had just moved under a file inside the checked scope, and only a human watching noticed; the mechanical defence is `tasks/PI-69`.
...
- **APPROVED:** `gh pr ready $N` · `gh pr comment $N --body "✅ Review passed — {n} criteria checked. {merge: orchestrator (default) | merge: user (draft requested)}"` (when you established a finding anyway, the comment also names it and its cause, in the same words as the report line) · `gh label create approved --color 22c55e 2>/dev/null || true; gh pr edit $N --add-label approved --remove-label needs-work 2>/dev/null` · task stays `Done` · `bash ~/.claude/agents/pocket-it/bin/handoff.sh log "{ID} PR #{N} approved — merge: {orchestrator|user}{ — cause: {example-not-class|base-moved|verification-reintroduced|false-coverage-evidence|other: …}}"` — the trailing `cause:` field only when this review established a finding of its own, in that exact shape, and absent entirely when it did not (never `first-round`, never an empty value: `bin/retro-due.sh` reads this field off the line whatever the outcome word, and both of those read as "no cause named").
- **NEEDS WORK:** (also `handoff.sh log "{ID} PR #{N} needs work{ (delta N)} — {first finding, six words} — cause: {first-round|example-not-class|base-moved|verification-reintroduced|false-coverage-evidence|other: …}"` — the `(delta N)` qualifier only from a delta round onward, and the trailing `cause:` field always present, in this exact shape: `bin/retro-due.sh` (PI-35) reads (a)/(b) from it, and a line without it never produces a signal) `gh pr comment $N --body "$(cat <<'EOF' … EOF)"` leading with the one-line cause from Step 0, then the findings list (file:line · rule · fix), `gh label create needs-work --color ef4444 2>/dev/null || true; gh pr edit $N --add-label needs-work`, and `set_status "Needs Work"` on the task file (tolerant sed from the shared rules — both `**Status**:` and `**Status:**` forms). Never `gh pr merge`.
...
Line 1: CI mode (`none` / `dev` / `prod`), `verify.sh` result, and your own `Effort: {N} min elapsed — wt:{basename of your working directory}` in the one unit every report uses (shared rules §7: minutes, and `wt:` copied from `basename "$PWD"` — never an agent id, you cannot read your own; `Effort: not available — no start timestamp` if you did not take the clock, never seconds or turns in its place). Then one line per PR: `{ID} — PR #{N} — APPROVED (merge: orchestrator|user){ — cause: {…} — fix at: {…}} | NEEDS WORK: {n} findings, first: {six words} — cause: {first-round|example-not-class|base-moved|verification-reintroduced|false-coverage-evidence|other: …} — fix at: {n/a|task file|developer prompt|shared/implementing-common.md|…}` — the same taxonomy on both branches, and on the APPROVED branch the `cause:`/`fix at:` pair appears exactly when this review established a finding of its own and is absent otherwise (Step 0: a cause is a property of the finding, not of the verdict). One line for conflicts resolved / CI fixes dispatched, if any.
