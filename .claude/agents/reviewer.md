...
**When this batch has more than one PR against the same base**, `verify.sh` alone will not catch a base moved under one of them: it runs each branch as it stands, and a shared value changed by one PR breaking another's tests leaves no textual conflict to flag it. After each PR's own `verify.sh` is green, build the merged tree once in a throwaway worktree — merge every not-yet-rejected PR of this batch onto the base, in review order — and run lint, type-check and the full unit suite on the result before approving any of them. A red merged tree is a NEEDS WORK on the PR that caused it, cause `base-moved`, with the merged-tree failure as the finding.
...
Line 1: CI mode (`none` / `dev` / `prod`) and `verify.sh` result. Then one line per PR: `{ID} — PR #{N} — APPROVED (merge: orchestrator|user) | NEEDS WORK: {n} findings, first: {six words} — cause: {first-round|example-not-class|base-moved|verification-reintroduced|other: …} — fix at: {n/a|task file|developer prompt|shared/implementing-common.md + reviewer.md Step 3|…}`. One line for conflicts resolved / CI fixes dispatched, if any.
