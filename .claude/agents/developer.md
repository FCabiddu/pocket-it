...
Read it. It is self-contained: goal, acceptance criteria (Given/When/Then), files, tests expected, TAD sections, risk, **budget** (minutes of wall clock; shared rules §7-§8 — an expectation, you stop on stall, not on size), notes. If it is missing or has no `**TAD**:` line, stop and report a planner gap — do not compensate by reading the IPD. Then read this project's facts, `handoff.sh facts` (shared rules §3, the whole facts section, up to the cap of 100 one-line facts): they are the mistakes already paid for on this project.
...
**Frontend — Motion:** implement the Design Spec §6 Motion System as specified — its dial level, tokens, technique map (catalogue numbers) and page-transition mechanism; never invent an animation outside it, never animate the surfaces it lists under "where not to animate". Rules: a `@media (prefers-reduced-motion: reduce)` branch on every animated component is mandatory (final state rendered, nothing infinite runs); animate `transform`/`opacity`/`clip-path` only — no `top/left/width/height/box-shadow` transitions; reveals through one `IntersectionObserver` that adds the visible class and **unobserves after the first reveal**; stagger delay from `--stagger-step` with the index clamped, `style={{ '--i': Math.min(i, N) }}` where `N` is the spec's cap; infinite animations (marquee, tickers) pausable — a visible pause/stop control plus pause on hover/focus (WCAG SC 2.2.2); durations and easings from the motion tokens, never literals; no motion dependency unless TAD 7.4 names one. **Signature moments** (Design Spec §6 table, compass #16–#27) are implemented exactly as their entry says — that library (or CSS 3D, no library), that fallback (reduced-motion, no-WebGL, mobile) as a real branch, lazy-loaded as 7.4 rules (`next/dynamic`, `ssr: false` for WebGL, poster rendered by the server) — never improvised, substituted or added where the spec has none. Tests: every animated component gets a test for the reduced-motion branch (final state rendered, no animation class) and one that the reveal class toggles on intersection (mock `IntersectionObserver`); a marquee gets a test that the pause control stops it.
...
Fix failures. Three rounds on the same error, or 20 minutes without a new commit or a new green test → stall: stop and report (shared rules §7). Over budget but progressing → log `BUDGET …` and finish.
