...
The compass holds the distilled *visual language* (colour, type, layout, motion). Modes A and B lean on it directly. Mode C **expresses the compass's colour and type principles as a token system**, **picks its motion from the Animation Catalogue and its signature moments from the Signature Moments section, both by technique number** (§6), and adds the enterprise layer below (components, screens, accessibility) that the compass does not cover — never restate compass content, build on it.
...
- **Dial.** Read `motion` from `.pocket-it.json` (or the BRIEF): `none` | `sober` (default) | `expressive`. State the level at the top of the sub-section. `none` = feedback transitions only (hover/focus/state changes on `--motion-fast`, no reveals, no page transitions). `sober` = feedback + page transitions + **4 catalogue techniques** on the public and first-run surfaces, on-scroll reveals once, stagger cap 6, no cursor-driven effects. `expressive` = `sober` + up to **6 techniques**, cursor- and scroll-driven ones allowed (#5, #12, #14), stagger cap 10, shared-element page transitions, **and 1–2 signature moments (mandatory)** picked by number from the compass **Signature Moments** section (#16–#27). `sober` **may** include **one** CSS-only signature moment (#16, #18, #23, #24, #25 — no library); `none` has none.
...
- **Signature moments** (at `expressive`, optionally one at `sober`). A second table `Surface → #n moment → metaphor → trigger → library (or `none`) → assets from the client → budget (KB gz, effort S/M/L) → fallback (reduced-motion / no-WebGL / mobile)`, one row per moment, copied from the compass entry and made concrete for *this* product. Place them where they matter — the home hero and the flagship product/detail page — never one per section; the metaphor must be the product's own physics (a book opens, a box lifts, an object turns). Prefer the 0-KB CSS-3D entries (#16, #18, #24, #25) before #19/#27; anything with frames, GLBs or `.riv` files gets the assets written out, because the client rarely has them. Any moment that needs a library is a §8 recommendation for TAD 7.4, not a decision.
...
- **Constraints**, non-negotiable: animate `transform`, `opacity`, `clip-path` only; `prefers-reduced-motion: reduce` honoured everywhere (reveals render in their final state, page transitions off, marquee static) and **no information conveyed only by motion**; anything infinite (marquee, tickers) gets a visible pause/stop control and pauses on hover/focus (WCAG SC 2.2.2); stagger index capped; no new motion dependency — if one is genuinely needed (a signature-moment library: page-flip, GSAP ScrollTrigger, model-viewer, Lottie/Rive, R3F, Spline), recommend it in §8 with its bundle cost and lazy-load rule for the TAD to approve or reject; nothing beyond feedback on the "where not" list.
