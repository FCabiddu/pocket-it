...
- **Display line-height**: tighten to `0.88`–`1.0` for headlines. Default browser line-height is for body copy, not display — but there is no formula that turns a font into a safe number. A font's published ascent+descent do **not** predict where its ink actually falls: measured in a real browser, Avenir Next (ascent+descent 1.366 em) renders `gjpqy` clean at line-height `1.12`, while Rockwell (ascent+descent 1.0 em) clips the same text at `1.0`, and Helvetica clips an accented capital (`È`) at `1.0` while its unaccented caps stay clean at `0.90`. Metric sums and per-family ranges (a "grotesque floor", a "serif floor") both under- and over-shoot depending on which glyphs the actual headline uses — a plain unaccented all-caps line tolerates a much tighter value than the same font set in lowercase with descenders, or in a language whose capitals carry diacritics. **There is no shortcut: render the real headline text, in the real font, at the candidate line-height, inside the real wrapper, and check the pixels** — not the metrics tables, and not a rule of thumb from another project.
- **No clipped glyphs, ever.** Ink may leave a glyph's own box on any of the four sides — below the baseline (descenders), above the cap-height (ascenders, accented capitals, diacritics), and left/right of the advance width (italic overhang, swashes, connected scripts) — and none of it may be cut, at any viewport width, by any property that clips rendering. That is a **class**, not the two members it is easiest to reach for; name it in full, across the whole page — on the checked element itself, on any ancestor, or on any descendant, never the ancestor chain alone (see the whole-page scope in **Glyph-clipping verification** below) — before checking: every `overflow` value that clips (`hidden`, `clip`, `auto`/`scroll` paired with a fixed or `max-height`, `-webkit-line-clamp`) and `overflow-clip-margin`; `clip-path` at rest and `clip: rect()` on a positioned element; `contain: paint | content | strict`; `content-visibility: auto | hidden`; `mask`, `mask-image`, `-webkit-mask-*`, `mask-border`. **Neutralising only `overflow` and `clip-path` is not a partial version of the check, it is a different, wrong one:** measured — with `contain: paint`, `contain: content`, `mask-image`, `-webkit-mask-image` or a positioned `clip: rect()` doing the clipping, the shipped shot and one with only `overflow: visible; clip-path: none` forced show **0** differing pixels while the full class reset finds **2196–2197** — five members the partial check misses every time. Verify with the one method in **Glyph-clipping verification** below; every other file that checks for clipped text cites that section by name, not its own variant.
...
### Glyph-clipping verification

One method, no alternative and no threshold. A metrics comparison (`canvas.measureText().actualBoundingBoxAscent`/`actualBoundingBoxDescent` against the box, `getBoundingClientRect()`, `Range` rects, `scrollHeight`/`clientHeight`) is not a second opinion on this and must not be used even as a fallback: measured, Rockwell at line-height `1` clipping `gjpqy` by 89 px in the screenshot still measures ink at `94.97px` against a `96px` box — under the box, so the metrics check calls it clean while the pixels show the cut.

**Procedure:**
1. Render the page as shipped. Screenshot one fixed region that covers the whole text with margin on all four sides (not just the element's own box — a clip can push ink outside it), in a real browser.
2. Render the same page again, same region, with the whole clipper class from **No clipped glyphs** above neutralised on **`*, *::before, *::after` across the whole page** (`overflow: visible`, `overflow-clip-margin: 0`, `clip-path: none`, `clip: auto`, `contain: none`, `content-visibility: visible`, `mask: none`, `mask-image: none`, `-webkit-mask-image: none`) — not just the checked element's ancestors, and not just the element itself. **A clip can sit on the element, on an ancestor, or on a descendant** (a masking wrapper *inside* the headline, exactly what technique #1 below builds) — scoping the reset to any one of those three misses the other two.
3. **In both shots**, before capturing, force `background`, `background-color`, `border-color` and `box-shadow` to `transparent` on `*, *::before, *::after` across the whole page, and hide non-text media in both — `img, video, canvas, svg, iframe { visibility: hidden }`. Only glyphs are being compared: skipping the transparency step counts a decorative shape near the text — a card background, an accent line — as a text clip; skipping the media step counts a clipped image or decorative line elsewhere in the region the same way, even when the text itself is untouched.
4. Diff the two screenshots pixel by pixel over the whole region. **Any differing pixel is a clip — no threshold, no percentage, no "close enough".**

Reproduced this round (Playwright/Chromium, threshold 0, whole-page scope): clean **0** · clip on an ancestor (`overflow: hidden` on a wrapper) **88** · clip on the checked element itself **2165** · technique #1's markup, mask on the inner descendant (`.line`, not the headline) **1131** — the three positions a clip can sit in all caught, none of them missed by scoping the reset too narrowly · a `clip-path` polygon (catalogue #13) that does not touch the text, on a block with an opaque background **0** · the same polygon actually cutting a letter **85** · a decorative line clipped by the same polygon, sibling to clean text **0** (no longer a false clip). Full table by technique, including the two standalone proofs at technique #1's line above, in the round-5 report.

**Exclusions.** The procedure above assumes every clip is unintended. Five techniques clip or hide **on purpose** — running step 2 unmodified on them erases the intentional truncation along with any real clip, but excluding the whole element is just as wrong: each excuses only the side(s) performing the truncation — one side for ellipsis, line-clamp and scrollers, both horizontal edges for the marquee, every side for `sr-only`. Every other side of the same element must stay checked exactly like the unmodified procedure. Apply the replacement only to the element actually performing the truncation; everything else in the region still gets the unmodified procedure above.

- **`text-overflow: ellipsis`** truncates the end of a line on purpose. Use `overflow: clip; overflow-clip-margin: 1em` on this element — not `overflow-x: clip; overflow-y: visible`, which also excused a real clip on the *start* side: measured, a bold capital's negative side-bearing overshoot at the very beginning of the title read **0** (clean) under the axis-wide version and **6/12** (a real clip) once fixed — two different test pages, DPR 1/2, same convention for every figure below. Also measured with the fix: a clean title **0/0**; the vertical axis, never truncated on purpose, still fully checked — a tight-line-height descender clip on it caught at **531/1853**.
- **The marquee track (catalogue #8)** is not the ellipsis case: both horizontal edges are meant to run past the box on purpose, so it keeps `overflow-x: clip; overflow-y: visible` unmodified — not re-measured this round.
- **`-webkit-line-clamp`** truncates below its last kept line on purpose, nothing else. A `clip-path` alone cannot express "below the Nth line": it does not know where a line boundary falls, so it either re-clips the kept lines' own descenders/accents or reopens the hidden lines. Measured wrong both ways with `clip-path: inset(-1em -1em 0 -1em)` alone: a descender cut on the last kept line — the most common line-clamp defect — read **0/0**; an accent cut on a kept line, with no `overflow: visible` in the reset at all, also read **0**. Correct procedure, on this element, **in both shots** — wrapping the words changes the pixel count by itself (measured 494/2841), so it must be present in the as-shipped shot too, not only the reset one:
  1. Wrap every word of the element's text in its own `<span>`.
  2. **Reset shot only:** group the spans into lines by `offsetTop`, and set `visibility: hidden` on every span whose line is at or past the computed `-webkit-line-clamp` value N — this only decides what to hide, it does not decide anything else. Then apply the generic whole-page reset to the element itself with no exception (`overflow: visible` included).
  Measured: a clean clamped paragraph **0/0**, the same at fractional metrics (17.3px font-size, line-height 1.37) **0/0**, the accent cut caught at **88/194**, the last-line descender cut caught at **385/1131**.
- **Scrollable containers** (`overflow: auto`/`scroll`) exclude only the side content scrolls toward — the opposite side, and both sides of the cross-axis, stay checked. Switching the container from `auto`/`scroll` to `clip` changes a handful of anti-aliased pixels by itself (measured 8/6), even with nothing hidden — make that switch **in both shots**, not only the reset one. In the **reset shot only**, add `overflow: visible; clip-path: inset(-1em -1em 0 -1em) padding-box` for a container that scrolls **vertically** (the `0` on the bottom keeps only the reachable-by-scrolling content below excused) or `clip-path: inset(-1em 0 -1em -1em) padding-box` for one that scrolls **horizontally** (the `0` on the right). **The `padding-box` is required, not decoration:** `overflow` clips a scrolling container at its padding-box, but `clip-path: inset()` defaults to the border-box — on a bordered container the two boxes sit `border-width` apart, so the unmodified reset re-clips the excused side further out than the shipped page ever did, reopening a strip the width of the border where scrolled-off content shows through. Measured on a container with a `2px` border: without `padding-box`, a clean page (only the intentional, excused truncation active, no real cut) read **291/1024** (vertical) and **28/110** (horizontal) — a false positive from the box mismatch, not from any ink; with `padding-box` added, both read **0/0**, and a real cut on the non-excused side is caught identically either way (unaffected, since that side already carries the generous `-1em`, far wider than any border). Measured without a border (the case that motivated the original fix): vertical — a real top-edge accent cut caught at **74/193**, clean at **0/0**; horizontal — a real cut caught at **228/693**, clean at **0/0**.
- **`sr-only`/visually-hidden text** (`clip: rect(0,0,0,0)`, 1×1px patterns): hidden by design on every side, not a clip to prove or disprove on any of them — exclude the element and its descendants from the reset entirely, e.g. give step 2's selector a `:not(.sr-only)` (name the project's own class or attribute). Measured: a clean control **0**.
...
**How:** Wrap each line: `<span class="line"><span class="line-inner">text</span></span>`. Outer: `overflow: hidden; display: block`. Inner: starts `translateY(110%)`, animates to `translateY(0)` via `@keyframes` or `IntersectionObserver` + CSS transition. Stagger lines with `animation-delay` or `transition-delay`. **The outer mask is permanent, not just for the animation's duration — if the line-height is tight, give the mask room on every side ink can leave from, without changing the visual line spacing.** `padding-bottom` alone does not do this: measured on a tight serif, `padding-bottom: .2em` alone left the top of an accented capital still clipped, and moved the line-to-line spacing from `0.92em` to `1.12em` (padding grows the box and nothing shrinks it back). What holds, measured on the same case (a serif with descenders and an accented capital, at line-height `0.92`, and a sans headline at `0.90`): `padding: .25em 0` on the outer (room on **both** top and bottom) plus `margin-bottom: -.5em` (**all** of the negative margin on one side, sized to the total padding added) — the line-to-line pitch stayed at the original tight value on both fonts and nothing clipped. Splitting the negative margin instead (`margin: -.2em 0`, top and bottom) does not fully collapse between adjacent lines and widens the pitch anyway — put it all on one side. And once the mask has this extra room, `translateY(110%)` is no longer enough to keep the resting line hidden — measured: the padded ink shows through before the reveal starts. **The safe `X` is the total vertical padding added — top plus bottom, not one side.** Sweeping `X` against `padding: .25em 0` (total `.5em`) on five fonts (Georgia, Baskerville, Didot, Helvetica, Futura), with an accented-capital, descender and swash-bearing string: `X = .25em` and `X = .35em` — a single side's worth — still show ink at rest on **every** font; only from `X = .5em`, the sum of both sides, does the line stay hidden on all five, and `X = .6em` stays safely hidden too. Tie `X` to the padding with one custom property instead of two numbers that can drift apart — e.g. `--line-pad: .25em` used in both `padding: var(--line-pad) 0` and `translateY(calc(100% + var(--line-pad) * 2))`. **Three different things need checking here, and only one of them is Glyph-clipping verification:**
- **Final state, after the reveal plays:** **Glyph-clipping verification**, as above.
- **Hidden at rest, before the reveal plays:** a different proof — the resting shot must be pixel-identical (0 differing pixels) to the same shot with `* { color: transparent }` forced. Glyph-clipping verification cannot be reused here: it compares as-shipped against clipper-neutralised, and at rest the clipper is doing exactly its job (hiding the line) — neutralising it would always show a difference whether or not the line properly hides.
- **Pitch unchanged, line to line:** a layout measurement, not an ink one — the difference in `offsetTop` between two consecutive `.line` elements must equal `line-height × font-size`, within the 1px rounding CSS layout allows. This is the one DOM/layout number this file endorses, and only for this: Glyph-clipping verification (pixels) cannot distinguish a widened pitch from a clean one, and a pitch is spacing, not ink — the false negatives that ruled out `getBoundingClientRect`/`scrollHeight` for clipping do not apply to measuring a gap between boxes.
...
**How:** Start `clip-path: inset(0 100% 0 0)`, animate to `clip-path: inset(0 0% 0 0)`. Pure CSS `@keyframes` or triggered by `IntersectionObserver`. Works on images, text blocks, coloured panels. **`inset(0 0% 0 0)` is not "no clip" — it is a clip drawn exactly at the element's own box edges, held there forever if the animation rests on its final frame.** On text, that box can be smaller than the ink on any of its four sides — not just the bottom: an italic's overhanging curves and swashes reach past the left/right edges the same way a descender reaches past the bottom or an accent past the top, and `clip-path` cuts all of them once applied, on a box that plain text (with no clip-path at all) would have been allowed to overflow unclipped. Measured on an italic serif with a descender and an accent: resting on `inset(0 0% 0 0)` clips it; resting on a small negative inset instead — `clip-path: inset(-.4em)` — gave the box room on all four sides and clipped nothing. Either rest the animation on a negative inset sized to the actual overhang (verify per **Glyph-clipping verification** — the right amount is font- and style-specific, not a constant), or drop to `clip-path: none` once the reveal completes instead of resting on the wipe's own end value.
...
**How:** Duplicate list items so total width is ~2×. Animate `transform: translateX(-50%)` from `translateX(0)` with `animation: marquee Xs linear infinite`. Parent: `overflow: hidden`. Pure CSS, zero JS. The parent's `overflow: hidden` is permanent here too, the same mask as #1 turned sideways — if the ticker text uses a tight line-height, give the parent the same vertical room (padding + one-sided compensating negative margin, see #1) or every pass of the marquee clips its own descenders and accented capitals.
...
**How:** `clip-path: polygon(0 0, 100% 0, 100% 100%, 0 100%)` base. On `:hover`: `clip-path: polygon(5% 0, 100% 3%, 95% 100%, 0 97%)`. CSS `transition: clip-path 0.4s ease`. Both must have the same number of points. If the element carries text (a "feature block" caption, not just an image), that hover polygon insets every edge by 3–5% of the box for the whole hover duration — enough to cut ascenders, descenders or accents sitting near an edge. Keep this technique to elements whose text sits with real margin from all four sides, or where only an image is inside the shape, not a headline.
...
- Any technique that masks or clips text — #1's and #8's `overflow: hidden`, #2's and #13's `clip-path`, or any other reveal or hover built the same way — must obey the no-clipped-glyphs rule in Typography principles, checked with **Glyph-clipping verification**, at rest after the animation or hover state has settled, not just mid-motion.
