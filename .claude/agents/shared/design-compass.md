...
**Procedure:**
1. Render the element as shipped. Screenshot one fixed region that covers the whole text with margin on all four sides (not just the element's own box — a clip can push ink outside it), in a real browser.
2. Render the same markup again, same region, with the whole clipper class from **No clipped glyphs** above neutralised on every ancestor (`overflow: visible`, `overflow-clip-margin: 0`, `clip-path: none`, `clip: auto`, `contain: none`, `content-visibility: visible`, `mask: none`, `mask-image: none`, `-webkit-mask-image: none`).
3. **In both shots**, before capturing, force `background`, `background-color`, `border-color` and `box-shadow` to `transparent` on the element and its ancestors. Skipping this step counts a decorative shape near the text — a card background, an accent line — as a difference that has nothing to do with the glyphs.
...
Reproduced this round (Playwright/Chromium, threshold 0):
- Clean case, no clipper active: **0** differing pixels.
- A `clip-path` polygon (catalogue #13) that does not touch the text, on a block with a background: **10,414** differing pixels with the backgrounds left opaque — a false clip, from the background shifting under the polygon, not from the text. **0** once background/border/shadow are transparent in both shots, per step 3.
- The same polygon actually cutting a letter: **1,251** differing pixels.
