...
- **No clipped glyphs, ever — verified on pixels, not on the DOM box.** Ink may leave a glyph's own box on any of the four sides — below the baseline (descenders), above the cap-height (ascenders, accented capitals, diacritics), and left/right of the advance width (italic overhang, swashes, connected scripts) — and none of it may be cut, at any viewport width, by any property that clips rendering. That is a **class**, not the two members it is easiest to reach for; name it in full, on every ancestor, before checking pixels: every `overflow` value that clips (`hidden`, `clip`, `auto`/`scroll` paired with a fixed or `max-height`, `-webkit-line-clamp`) and `overflow-clip-margin`; `clip-path` at rest and `clip: rect()` on a positioned element; `contain: paint | content | strict`; `content-visibility: auto | hidden`; `mask`, `mask-image`, `-webkit-mask-*`, `mask-border`. **Neutralising only `overflow` and `clip-path` is not a partial version of this check, it is a different, wrong one:** measured — with `contain: paint`, `contain: strict`, `content-visibility: hidden`, `mask-image` or a positioned `clip: rect()` doing the clipping, a screenshot with only `overflow: visible; clip-path: none` forced is pixel-identical to the shipped, clipped screenshot (diff < 30px on a 260×260 crop) while the full class reset (all of the above, together) uncovers the missing ink (1.4k–4.2k px of new, previously-cut content) — five ways for the two-property check to call a cut headline clean. Check by rendering the element twice, once as shipped and once with the whole class above reset on every ancestor, and comparing screenshots (or comparing the font's real ink extent from `canvas.measureText(text).actualBoundingBoxAscent`/`actualBoundingBoxDescent` against the box's own geometry). **`getBoundingClientRect()`, `Range` rects and `scrollHeight`/`clientHeight` measure the metrics box, not the ink, and disagree with pixels in both directions** — measured: Rockwell at line-height `1.0` clips in the screenshot while its range rect and `scrollHeight` both say nothing overflows (false negative); Avenir Next at `1.12` clips nothing in the screenshot while both DOM signals say it overflows (false positive). A DOM measurement is not a substitute for a pixel check.
...
- Any technique that masks or clips text — #1's and #8's `overflow: hidden`, #2's and #13's `clip-path`, or any other reveal or hover built the same way — must obey the no-clipped-glyphs rule in Typography principles: verified on pixels, at rest after the animation or hover state has settled, not just mid-motion, and not from the DOM box.
