...
**Exclusions.** The procedure above assumes every clip is unintended. Three classes clip **on purpose**, using the very properties step 2 neutralises — running step 2 unmodified on them proves nothing (it erases the intentional truncation along with any real clip) and running the full page-wide reset on them at all is the wrong question. Each needs its own handling in place of the generic reset, still at zero threshold, applied only to the element actually doing the intentional truncation — everything else in the region still gets the unmodified procedure above:
- **`text-overflow: ellipsis` and the marquee track (catalogue #8):** truncate horizontally on purpose. Do not reset `overflow` to `visible` on this element; use `overflow-x: clip; overflow-y: visible` instead. Plain `overflow-x: hidden` does not work here — pairing a non-`visible` `overflow-x` with a `visible` `overflow-y` promotes the `visible` axis to `auto`, which still clips; `clip` is exempt from that promotion. Measured: a clean title **0**, the same title with its descenders clipped by a tight line-height **623**.
- **`-webkit-line-clamp`:** truncates below its last line on purpose. Replace the reset with `clip-path: inset(-1em -1em 0 -1em)` on this element — it keeps the below-the-last-line cut while opening enough room on the other three sides for real ink on the kept lines to show. Measured: a clean clamped paragraph **0**, the same paragraph with its kept lines clipped by a tight line-height **111**.
- **`sr-only`/visually-hidden text (`clip: rect(0,0,0,0)`, 1×1px patterns) and scrollable containers (`overflow: auto`/`scroll`, the rest of the content reachable by scrolling):** hidden or reachable by design, not a clip to prove or disprove — exclude the element and its descendants from the reset entirely, e.g. give step 2's selector a `:not(.sr-only)`/`:not(.scroll-container)` (name the project's own class or attribute). Measured: a clean `sr-only` control **0**, a clean scrollable container **0**.
