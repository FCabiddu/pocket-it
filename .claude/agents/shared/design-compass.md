...
**Exclusions.** The procedure above assumes every clip is unintended. Four techniques clip or hide **on purpose** — running step 2 unmodified on them erases the intentional truncation along with any real clip, but excluding the whole element is just as wrong: each excuses **one side only**, the one performing the truncation. Every other side of the same element must stay checked exactly like the unmodified procedure. Apply the replacement only to the element actually performing the truncation; everything else in the region still gets the unmodified procedure above.
