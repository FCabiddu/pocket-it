...
The orchestrator's context is the most expensive thing in the pipeline, so what you return is short and what you write down is complete. Before your final message, write `docs/reports/{ID}-{YYYY-MM-DD}.md` (≤ 40 lines: what changed and why, tests ↔ criteria, deviations, checks run with their result, turns used, anything the next agent needs) and commit it with your task file so it travels in the PR. **Turns: measure the three things you can measure, never estimate and never write "not counted".** You have no access to your own turn counter, so a number you produce from memory is invented — four reports in one two-day window carried either an estimate or "not counted", and on the day the first real count was finally taken two reports that had claimed to be within budget were at about twice their real turns. A number carrying `~`, "about", "roughly" or a range **is an estimate**, whatever the sentence around it claims — one report wrote "~31 turns" while stating it had counted them. What you can measure exactly: **elapsed minutes** (`date +%s` in your first command and again in your last), **commits** (`git rev-list --count "origin/$BASE"..HEAD`), and **your own agent id**, which is the last path segment of your worktree (`.../.claude/worktrees/agent-XXXXXX`). Write those three in the report's Turns section and in any `BUDGET`/`STALL` line; `usage-report.py` turns the id into the true turn count for whoever reads them later. Then return **at most 8 lines**: ID and status · branch and PR URL · tests added and scoped-run result · elapsed/commits/agent id against the budget and whether `BUDGET`/`STALL` was logged · blockers or deviations in one line · `Report: docs/reports/{ID}-{date}.md`. No prose, no explanations — they are in the file.
...
- **Budget exceeded while progressing** (commits landing, tests turning green): keep going. Log once — `bash ~/.claude/agents/pocket-it/bin/handoff.sh log "BUDGET {ID} {elapsed} min / {commits} commits / agent-{id} vs budget {budget} — progressing: {tests} tests green — {why bigger than estimated}"` — and finish. The retro reads these lines to correct the estimates.
- **Stall**: no new commit and no additional passing test in the last ~30 turns (in elapsed terms, roughly the last 15 minutes with nothing landing), or the same error surviving three fix attempts. Stop: commit what is coherent, push, log `handoff.sh log "STALL {ID} ~{turns} turns — {what is stuck, one line}"` and report. A clean partial report is cheaper than a runaway.
