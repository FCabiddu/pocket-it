...
```
Effort: 23 min elapsed (budget 90 min) — agent-acee2c67
```

`date +%s` in your **first** command and again in your **last**, difference divided by 60, **rounded to a whole minute**. Wall clock, waiting included; that is what it is, and it is the same for everyone. The budget in brackets is the task's, in minutes (§8). `agent-…` is the last path segment of your worktree — an **identifier, not a measure**: it is the key `usage-report.py` uses to look up your true turn count later, which is why the id makes the whole thing verifiable and nothing else needs to be guessed.
...
**When the number is not available, say so — never substitute another quantity in silence.** If you did not take the starting timestamp, the line is exactly `Effort: not available — no start timestamp` (add `— agent-…` if you know your id, so the real count can still be recovered). Seconds, commits, tool calls and "it felt like" are not fallbacks for minutes: a missing number costs the retro one data point, a silently swapped unit costs it the comparison across every report in the window. Commits belong to the Changes section, not here.
...
- **Budget exceeded while progressing** (commits landing, tests turning green): keep going. Log once — `bash ~/.claude/agents/pocket-it/bin/handoff.sh log "BUDGET {ID} {elapsed} min elapsed vs budget {budget} min — agent-{id} — progressing: {tests} tests green — {why bigger than estimated}"` — and finish. The retro reads these lines to correct the estimates.
- **Stall**: **20 minutes** of wall clock with no new commit and no additional passing test, or the same error surviving three fix attempts. Both are things you can check: the clock, and your own count of attempts on one error. Stop: commit what is coherent, push, log `handoff.sh log "STALL {ID} {elapsed} min elapsed — agent-{id} — {what is stuck, one line}"` and report. A clean partial report is cheaper than a runaway.
