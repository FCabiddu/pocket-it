...
**Worktree gone — stop, never fall back.** The shell's working directory does not persist between commands: it resets to where the session started, usually the project's main checkout. So begin every command with an explicit `cd <your worktree> && …` — chained with `&&`, never `;`, so that nothing runs when the `cd` fails. Your worktree is lost when its path no longer exists, when `git rev-parse --show-toplevel` prints another path, or when `git branch --show-current` prints another branch. Then **stop and report**: the path, the branch, the last commit you pushed, what was left uncommitted. Do not recreate the worktree, do not check the branch out elsewhere, and **do not continue in the main checkout or in any other directory** — not for a one-line edit, not for a test run, not for `handoff.sh`. A change made there lands in the tree every other session and every hook reads, with no branch, no review and no PR; lost uncommitted work is recoverable, an unreviewed edit to the shared tree may not be noticed at all. A worktree can vanish under a running agent (a cleanup after someone else's merge, a manual prune): commit early, uncommitted work is exactly what it takes with it.
