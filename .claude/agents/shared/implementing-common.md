...
**Real** describes the engine: an actual database running real queries (Postgres, MySQL, SQLite on disk) — never a mock, a stub or an in-memory fake. It says nothing about who else can see it. **Disposable** is real-and-not-shared: a database you created inside this run and remove before this run ends — a container you start and remove, a file you create and delete, a database you `createdb` and `dropdb` — holding only data you put there, even when a config file such as `.env.test` or a test compose file points at it. **Shared** is every other database: one that existed before this run, or one you create and leave behind, whatever points at it. The unit is the database you connect to — what a container, a file or `createdb` creates — not the server it runs on: a database you create on a server that was already running can be disposable, while every other database on that server stays shared. Who created it, and whether it is gone before this run ends, decide; a config file never does. *This run* is your agent run, from launch to your final report. Every `real DB` / `live DB` mention elsewhere in this file means *real-and-disposable* — a genuine engine created for the run, isolated per rollback/truncate, never the shared one; that is also what `qa-engineer.md`'s integration tests run against. **Realistic data**, for whoever verifies a fix, means realistic rows seeded on a disposable database — never a write, rolled back or not, to a shared one (rules below). If you cannot create a disposable database here (no container runtime, the project only configures a shared instance), do not substitute a mock or another engine and do not skip silently: stop, push what you have, and report `blocked — no disposable database: {why}` (§8).

As a check, not a rule of its own: a test container this run starts and leaves running is shared — remove it before you finish; a test database kept from earlier runs is shared — create a fresh one for this run; a database you `createdb` on the local server for this run and `dropdb` at the end is disposable, while the development database next to it is shared; an existing staging database is shared, and restoring a dump over it is a write to a shared database.
