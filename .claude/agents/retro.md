...
**Retro-mark — the last thing you do, after every PR above is merged.** `bin/retro-due.sh` (`PI-35`) counts signals — repeat review causes, tasks stuck at three-plus rounds, `BUDGET`/`STALL` lines — since the last time a retro ran; this line is what tells it a retro just ran. Once the project PR (facts + best-practices) is merged — and the pocket-it lessons PR too, when one was opened — on the project's base branch: `git checkout {baseBranch} && git pull --ff-only origin {baseBranch} && git checkout -b retro/{scope}-{date}-mark`, `bash ~/.claude/agents/pocket-it/bin/handoff.sh log "retro-mark $(date +%Y-%m-%d) {scope}"`, commit `docs/SESSION_HANDOFF.md` (and `docs/SESSION_HANDOFF_ARCHIVE.md` if the rotation touched it), push, `gh pr create --base {baseBranch} --title "retro-mark: {scope}"`, then `POCKET_IT_USER_MERGE=1 gh pr merge {n} --squash --delete-branch` — same self-merge authority as the text-only PRs above, never a direct push to the base branch. Skip only when the PRs above stayed in draft (config `automerge: false`, or `Draft: yes` requested): nothing was actually merged yet, so no signal has been addressed and the count must stand.
