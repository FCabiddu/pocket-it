...
- **A class of cases is specified as the class, never by examples.** When a task covers a set — command shapes a guard reads, an argument grammar, id shapes, a category definition, a verification procedure, the commands a script prints — its criteria list the class's dimensions (each axis and the values it takes) with a table of cases (combination → expected outcome), written into the task from the start, prose-only and definition tasks included. Its `Tests expected` says the test is generated from the same declaration that defines the behaviour (iterate the alias list, the per-subcommand argument declaration; execute every printed command on names with shell-special characters), never a hand-picked sample. Invariants are written as the damage to prevent ("nothing written to any remote, under any name"), never as the place it was seen ("not on the base"). A guard or check task states its threat model in the Goal: what it protects against, and what it deliberately leaves to another layer. Measured: four such tasks, each specified by examples, took 4, 5, 7 and 7 review rounds, each closing the cases it was shown; a guard with no threat model was cancelled after three.
...
- **Budget.** Every task carries `**Budget**: N min` of wall clock derived from the estimate (XS 15, S 45, M 120, L 180 — the 75th percentile of 163 measured developer runs, shared rules §8). Minutes, because that is the only unit an implementing agent can measure about itself and therefore the only one its report can be compared in (shared rules §7). **Write the unit in the field, not only in this table** (a mechanical refusal is `tasks/PI-67`): a bare number is read as a turn-era budget, and a turn budget does not convert — turns and elapsed minutes correlate at r=0.42, and the conversion once prescribed here set every XS and S threshold above the 90th percentile of real runs, so four consecutive windows produced no `BUDGET` line at all and no estimate correction could be derived from them. A size copied out of an old estimate fact (`S=120`, `M=200`, `L=300`) is a **turn-era size, not minutes**: three task files written in one day carried `Budget: 120 | 90 | 60` that way. It is an expectation, not a cap: developers stop on stall, not on size, and log `BUDGET …` when a task costs more while progressing. Before decomposing, read `~/.claude/agents/pocket-it/.claude/agents/shared/lessons.md` (method lessons valid on every project, e.g. what must not be split) and, before estimating, the last retro's estimate corrections among this project's facts (`handoff.sh facts`) and the `BUDGET` lines in its log (`handoff.sh grep "BUDGET|STALL"`): a kind of task that repeatedly ran over gets the next size up. Work that cannot be split (run a whole suite and fix the reds, a migration plus its backfill) gets a custom budget and a one-line reason in Notes.
- **Risk.** Every task carries `**Risk**: low | high`. High = migrations, auth/permissions, money, data deletion, anything irreversible or security-relevant. It feeds `next-wave.sh`'s `model_hint`, a starting suggestion, not the orchestrator's model decision — the orchestrator picks the model at every launch per this repo's own "Model choice" criteria (`CLAUDE.md`, `run-wave`/`quickfix` skills). Keep the flag honest all the same — an inflated one skews that suggestion for nothing, a deflated one hides work that needed the stronger model.
...
**Estimate**: XS | S | M | L
**Budget**: 45 min | 90 min | 150 min | 240 min   ← wall clock, from the estimate; the unit goes **in the value**, never only in this comment; a custom number for work that cannot be split (say why in Notes)
**Risk**: low | high
