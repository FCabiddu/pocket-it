...
- **Budget.** Every task carries `**Budget**: N min` of wall clock derived from the estimate (XS 45, S 90, M 150, L 240). Minutes, because that is the only unit an implementing agent can measure about itself and therefore the only one its report can be compared in (shared rules §7). **Write the unit in the field, not only in this table**: a bare number is read by the next agent as the turn-era budget it used to be and converted at 47 s/turn, and the two readings do not agree — the same bare `200` was read as 157 min by one agent and as 200 min by another, and the second reading turned a 60 % overrun into a report that declared itself within budget. It is an expectation, not a cap: developers stop on stall, not on size, and log `BUDGET …` when a task costs more while progressing. Before decomposing, read `~/.claude/agents/pocket-it/.claude/agents/shared/lessons.md` (method lessons valid on every project, e.g. what must not be split) and, before estimating, the last retro's estimate corrections among this project's facts (`handoff.sh facts`) and the `BUDGET` lines in its log (`handoff.sh grep "BUDGET|STALL"`): a kind of task that repeatedly ran over gets the next size up. Work that cannot be split (run a whole suite and fix the reds, a migration plus its backfill) gets a custom budget and a one-line reason in Notes.
