...
- **Budget.** Every task carries `**Budget**: N` turns derived from the estimate (XS 60, S 120, M 200, L 300). It is an expectation, not a cap: developers stop on stall, not on size, and log `BUDGET …` when a task costs more while progressing. Before decomposing, read `~/.claude/agents/pocket-it/.claude/agents/shared/lessons.md` (method lessons valid on every project, e.g. what must not be split) and, before estimating, the last retro's estimate corrections in `docs/SESSION_HANDOFF.md` facts and the `BUDGET` lines in its log: a kind of task that repeatedly ran over gets the next size up. Work that cannot be split (run a whole suite and fix the reds, a migration plus its backfill) gets a custom budget and a one-line reason in Notes.
...
**Estimate**: XS | S | M | L
**Budget**: 60 | 120 | 200 | 300   ← turns, from the estimate; a custom number for work that cannot be split (say why in Notes)
**Risk**: low | high
