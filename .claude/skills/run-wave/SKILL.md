...
- **Session hygiene.** The call to compact is yours, not the user's — it is not a question to put to them. You cannot make it happen from here either: no tool, hook or command in this skill starts a compaction, and the running session cannot read its own context usage — that stays with the runtime, never with you. What is yours is the call and the moment, and you judge it from signals you already have on disk, not from a context number you cannot see: the `wave … closed` lines logged today by this session, how many agents you launched across them, and how many NEEDS WORK / delta-review rounds it took. After the **second** closed wave, end the report with a statement, not a question: "Due wave chiuse in questa sessione: prima della prossima la sessione va compattata — lo stato è su disco, non si perde nulla." The user presses `/compact` because the tool needs a human keystroke, not because their opinion was being asked. Do not launch a third wave in the same session before that happens.
