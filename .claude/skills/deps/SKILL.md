...
### 2. Task file
Write `tasks/QF-{n}-deps-rollup-{YYYY-MM}.md` with the quickfix template (`Label: DevOps`, `Estimate: S`, `Budget: 90 min`, `Risk: low`). Goal: one branch that merges every listed dependabot branch, lockfile regenerated once, **full** test suite green once (this lane is the one exception to "affected tests only"), type-check and lint clean. Check `.github/dependabot.yml`: if `schedule.interval` is `weekly` and there is no `groups` block, the same task also converts it to monthly + grouped (one group for minor/patch, majors ungrouped) — add it to **Files**, it is engineering upkeep of the pipeline's own cadence, not a business choice, so it is implemented by the `developer` and reviewed like the rest of the task, never proposed to the user and never edited from this session. Acceptance criteria: AC1 every listed PR's bump is present in `package.json`/lockfile; AC2 full suite, type-check, lint green with output tails in the PR body; AC3 no source change outside `package.json`, the lockfile, `.github/dependabot.yml` (only when this task's condition triggered its edit) and files a codemod had to touch (listed and justified); AC4 (only when `dependabot.yml` qualified) it now reads `schedule.interval: monthly` with a `groups` block, minor/patch grouped and majors left ungrouped. Notes: the PR numbers and branch names; the command `git merge origin/<branch>` per PR, resolving lockfile conflicts by regenerating it, never by hand. Commit and push the task file (worktree rule from `/quickfix` §3).
