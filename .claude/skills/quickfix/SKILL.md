...
**Retro trigger — after every review, first round and any delta re-review alike.** `bash ~/.claude/agents/pocket-it/bin/retro-due.sh`. Exit 0 (`retro-due: nothing`) → continue. Exit 10 (`RETRO DUE: …`) → launch the `retro` agent right now, in background — `subagent_type: retro`, `run_in_background: true`, prompt = the script's signal lines as scope — never asking, never waiting for the epic to close. Exit 2 is a script or environment error, not a "no signals" answer: fix it, do not treat it as nothing.
