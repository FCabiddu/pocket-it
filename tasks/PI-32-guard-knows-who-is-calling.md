...
**PR**: https://github.com/FCabiddu/pocket-it/pull/54
...
## Acceptance criteria
- [x] AC1 — Dato un push verso il ramo base in un input che contiene `agent_id`, quando l'hook lo valuta, allora lo blocca **anche se il comando porta il prefisso di audit**, con un messaggio che dice che agli agenti il ramo base è vietato e dove vanno invece le righe di diario.
- [x] AC2 — Dato lo stesso push con il prefisso esatto in un input **senza** `agent_id`, quando l'hook lo valuta, allora passa come oggi.
- [x] AC3 — Dato un `gh pr merge` con il prefisso di merge in un input che contiene `agent_id`, quando l'hook lo valuta, allora lo blocca, **salvo** gli agenti che per mandato mergiano le proprie PR: oggi il solo `retro`, riconosciuto da `agent_type`.
- [x] AC4 — Dato un push verso un ramo di task in un input con `agent_id`, quando l'hook lo valuta, allora passa: agli agenti resta permesso tutto ciò che era permesso, tranne il ramo base.
- [x] AC5 — Dato un input con `agent_id` presente ma vuoto, o di tipo inatteso, quando l'hook lo valuta, allora lo tratta come agente: nel dubbio si nega.
- [x] AC6 — Il commento in testa all'hook spiega il meccanismo, la verifica fatta e perché il prefisso da solo non bastava.

- [x] AC7 — Dato un comando lanciato da un agente in cui la rimozione delle virgolette si sfasa, per esempio una stringa `$'…\'…'` seguita più avanti da un'altra stringa fra apici singoli che nasconde un push, quando l'hook lo valuta, allora il push verso il ramo base è comunque negato: per un agente la decisione non deve dipendere dalla lettura corretta del testo.
