...
Basta un solo task con un segmento alfabetico nell'id perché lo script si fermi. Poiché `next-wave.sh` serve all'avvio di ogni sessione e a ogni wave per decidere cosa è pronto, finché quel task esiste la pipeline è cieca. Gli id con un segmento alfabetico sono previsti da questo repo: `T-BUG-{n}` è la convenzione che `qa-engineer` usa per i bug, e la corsia quickfix usa `QF-{n}`.
