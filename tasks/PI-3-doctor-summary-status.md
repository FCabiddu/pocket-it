...
## Acceptance criteria
- [ ] AC1 — Given a folder `tasks/EPIC-*/` whose `EPIC.md` Status is not `Done` and whose every `T-*.md` child has Status starting with `Done`, when doctor runs, then it prints `warn  tasks/EPIC-…/EPIC.md: says "<status>" but all N children are Done — update the summary`.
- [ ] AC2 — Given a `STORY-n.m.md` in that folder whose children `T-n.m.*.md` are all Done and whose own Status is not Done, when doctor runs, then the same warning form is printed for the story.
- [ ] AC3 — Given a summary whose children are not all Done, or a folder with no children, when doctor runs, then no warning is printed for it.
- [ ] AC4 — Given a flat board (no `tasks/EPIC-*/` folders), when doctor runs, then output is unchanged and exit code unchanged.
- [ ] AC5 — Given `bin/doctor.test.sh` (new), when it runs on temporary boards built in a temp git repo, then it covers AC1–AC4, `ok`/`FAIL` per case, exit 1 on FAIL; add it to `testCommand`.
