...
## Acceptance criteria
- [x] AC1 — Dato un repository in cui il ramo base remoto è avanzato normalmente dall'ultima esecuzione, quando `doctor.sh` gira, allora non segnala nulla.
- [x] AC2 — Dato un repository in cui il ramo base remoto non contiene più il commit visto all'ultima esecuzione, cioè è stato riscritto, quando `doctor.sh` gira, allora segnala un **errore** con il commit perso, così da poterlo ripristinare.
- [x] AC3 — Dato un repository in cui il ramo base remoto non esiste più, quando `doctor.sh` gira, allora segnala un errore.
- [x] AC4 — Dato un repository alla prima esecuzione, senza un commit già visto, quando `doctor.sh` gira, allora registra il commit corrente e non segnala nulla.
- [x] AC5 — Il commit già visto è conservato in un posto che non viene committato e che ogni worktree dello stesso repository condivide, così una esecuzione da un worktree vede ciò che ha registrato un'altra.
- [x] AC6 — Il messaggio di errore dice come recuperare il ramo, con il commit da cui ripartire.
