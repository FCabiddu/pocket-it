...
- [ ] AC2 — Given `.claude/agents/shared/lessons.md`, when AC1 has landed, then line 31 is **deleted in the same commit** — not reworded, not marked `confirmed`. `git diff` shows the file one line shorter and nothing else changed in it.
...
- [ ] AC4 — Given the repo's 300-**byte** guard on lesson lines and the lesson count, when the change is done, then `awk '/^- 20/{if (length($0) > 300) print NR, length($0)}' .claude/agents/shared/lessons.md` prints nothing and the file holds 12 lessons (13 before).
