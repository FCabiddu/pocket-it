...
Per AC3 il confronto fra il riquadro DOM del testo (`getBoundingClientRect`/`scrollHeight` vs `clientHeight`) e quello dell'antenato che lo ritaglia **non è una misura valida**: misura il riquadro delle metriche del font, non l'inchiostro, e sbaglia in entrambe le direzioni (verificato: un'interlinea stretta taglia i discendenti mentre il rettangolo dice che non esce nulla; un'interlinea generosa fa scattare un falso rosso senza alcun taglio visibile). Anche `canvas.measureText()` sull'intera stringa sbaglia allo stesso modo su un titolo andato a capo: misura una riga sola larga più del contenitore su un titolo di due righe senza alcun taglio orizzontale reale, e non vede mai un taglio verticale sulla seconda riga. **L'unica misura valida è la posizione dei pixel** (screenshot di una regione fissa con e senza il ritaglio, confrontati, senza soglia di tolleranza: un pixel diverso fuori dal riquadro di padding dell'antenato è un taglio); `measureText` resta utilizzabile solo come filtro rapido riga per riga (baseline di ogni riga, non la stringa intera), mai come prova. Un controllo sulla presenza di una classe CSS non vale in ogni caso: è il genere di test rimasto verde con il difetto presente.
