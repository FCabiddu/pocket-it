...
**Ownership — who sets it.** `devops-engineer` *creates* the variable when it first writes the CI pipeline, idempotently and always at `dev` (`gh variable get APP_STATUS >/dev/null 2>&1 || gh variable set APP_STATUS --body dev`), so regenerating a workflow never silently resets a project that is already in `prod`. **Nothing else in the pipeline ever writes it.** Turning CI on is an explicit human action in every path: `tech-architect` documents that the switch exists but must not recommend a starting value or spec a project as starting in `prod`, and `reviewer` reads it and is forbidden from flipping it — at most it notes in its report that a PR looks worth a real CI run. Deleting the variable falls back to `dev`; the safe default is always "don't spend".
