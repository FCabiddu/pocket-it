...
- `shared/lessons.md` — method lessons, stack-independent, one line each in a fixed form (≤ 300 characters, with a `WHERE` naming the destination of its future rule), no client names. Read at Step 0 by developer, qa, reviewer, planner; written by `retro` (self-merged PR) and by humans. Lifecycle: a lesson is born `provisional`; the retro that sees it hold on a second epic or project **writes the rule at its `WHERE` and deletes the line in the same PR** — confirming is promoting, there is no `confirmed` state resting in the file and no human step; contradicted or already-written-elsewhere lessons are deleted with the reason in the PR. The cap of 40 lines is a symptom that must never be reached.
