...
| `retro.md` | Opus | 60 | End of epic (or on signals): repeated findings → best-practices rules and project facts (PR) + method lessons, promoting a lesson that held into this repo's own templates in the same PR |
...
| `verify.sh <PR|branch>` | lint, type-check, affected tests on the PR branch in a throwaway worktree, ≤ 40 lines. The reviewer runs it before reading. Exit 0 green, 1 the branch’s own red, 3 a red that also fails at the base commit it pinned for the run (re-run there, once, only for the checks that failed, and only when it could establish the base can run them at all), 2 could not run — including an installed dependency tree that disagrees with the lockfile **or that could not be compared with it at all** (a lockfile format install-drift.sh cannot read: pnpm, yarn, bun, an unparsable or unknown-shape file). Both are refused before any check runs and never become a GREEN: a verdict about code that is not installed, or about a tree nobody compared, is not a verdict |
| `install-drift.sh check|reinstall|lockfiles [dir]` | one question each way: `check` compares the installed `node_modules` with the lockfile (npm formats v1/v2/v3; any other package manager says "drift check not supported", never a silent pass) — exit 0 match, 1 drift, 3 no verdict (`verify.sh` turns both 1 and 3 into its own exit 2: an uncompared tree is never a green); `reinstall` runs the lockfile's own frozen install (`npm ci`, `pnpm|yarn|bun` equivalents) in that checkout, but only when nothing is **running** against it — evidence is a live process whose working directory is in the checkout or one of its worktrees, or `--running <what>`, or an unreadable process/worktree list; a worktree that is merely registered is reported, not a blocker (worktree.sh locks at creation and only cleanup-merged.sh releases, so registration outlives its agent by days). A deferral goes to `handoff.sh log` and exits 4, and says out loud when that write did not happen. `--except <branch>` drops a worktree the caller has just merged. `lockfiles` prints the one declaration of lockfile names, which `verify.sh` reads instead of keeping its own. `verify.sh` calls `check`; the closing step of `/quickfix` and `/run-wave` calls `reinstall` after `cleanup-merged.sh`, when the merged PR touched a lockfile. Test: `install-drift.test.sh` |
...
| `handoff.sh log|fact|retract|show|where` | the narrative memory under `docs/handoff/`: every write creates one new immutable file (`log` a dated event, `fact` an evergreen fact, cap 100 visible, `retract` hides one), so two branches never contend; the reads compose the single view and touch nothing. Agents call it at every PR and review; humans read it after a week away |
| `tasks-index.sh` | regenerates `tasks/INDEX.md` |
| `cleanup-merged.sh [--dry-run] [--all]` | removes the developer/reviewer worktrees (`.claude/worktrees/agent-*`, `<parent>/.worktrees/<task>`) and local branches whose PR is merged (merged into `origin/main`/`origin/epic/*`, or remote branch gone and `gh` reports the PR merged), plus detached `/tmp` scratch older than 24 h; keeps and lists dirty, unmerged and `epic/*`/`main`/`fix-*` worktrees (`--all` includes the latter), never the current one; a worktree locked by `worktree.sh` (created locked, PI-31) is judged only by a merged PR containing its tip — then released and removed; when `gh` is missing or fails the line says the PRs are unknown, never "not merged"; every kept pocket-it lock prints its release command (`worktree.sh --unlock <repo> <branch>`), and every other lock is kept. Run from any worktree; `/run-wave` and `/quickfix` call it after the merge. Test: `cleanup-merged.test.sh` |
| `worktree.sh <repo> <branch> [base]` · `worktree.sh --unlock <repo> <branch>` | creates or reuses an agent's worktree at `<repo>/.claude/worktrees/<branch-slug>` from any cwd (local branch as-is, else tracking `origin/<branch>`, else off `origin/<base>`), prints only its path. The worktree is **locked** (reason `pocket-it: agent worktree for <branch> since <date>`) so `cleanup-merged.sh` never removes it before a merged PR contains its tip. `--unlock` releases that lock for abandoned work, a closed PR or a merge without a PR; a lock with another reason is left alone (exit 1). Test: `worktree.test.sh` |
...
- `shared/lessons.md` — method lessons, stack-independent, one line each in a fixed form (≤ 300 bytes, with a `WHERE` naming the destination of its future rule; `tasks/PI-53` is the guard that refuses the file otherwise), no client names. Read at Step 0 by developer, qa, reviewer, planner; written by `retro` (self-merged PR) and by humans. Lifecycle: a lesson is born `provisional`; the retro that sees it hold on a second epic or project **writes the rule at its `WHERE` and deletes the line in the same PR** — confirming is promoting, there is no `confirmed` state resting in the file and no human step; contradicted or already-written-elsewhere lessons are deleted with the reason in the PR. The cap of 40 lines is a symptom that must never be reached.
...
| History log | `docs/handoff/{AAAA-MM}/*.md`, **mandatory**, written only through `bin/handoff.sh`, one immutable file per write: `## Fatti` (≤ 100 visible, gotchas and decisions, pruned with `retract`) + `## Log` (every event kept; 40 is only what `show` displays). `docs/SESSION_HANDOFF.md` and its archive are frozen sources, read for ever, never written again. State is never written here — `status.sh` computes it |
| Project isolation | **Nothing from a project pocket-it works on ever lands in this repo.** No project name, no product or domain vocabulary, no file paths, no PR or issue numbers, no excerpts of its code, documents or data — in task files, reports, commit messages, PR bodies, `lessons.md` or the handoff memory. This repo is public and the projects are not. A tooling bug found while working on a project is described by its mechanism alone: the failing line, the input shape that triggers it, the expected behaviour. Id shapes this repo defines itself (`T-{e}.{s}.{t}`, `T-BUG-{n}`, `QF-{n}`, `PI-{n}`) are its own conventions, not project data, and stay usable as examples and test fixtures. When a diagnosis cannot be written without naming a project, it belongs in that project's report, and only the mechanism comes here |
...
| Cost rules | task file + cited sections only; read once; capped output; budget per task (stop on stall, not on size; `maxTurns` 300 is the safety net); reviewers grouped by measured cost, never one for an arbitrarily large wave; a stopped agent or a red PR gets its cause fixed before anything is relaunched, never a bigger model as the first move — resume the same agent unless the analysis says otherwise, relaunch from scratch only when the worktree is gone; compaction is automatic (`autoCompactWindow`), nothing waits for it and state stays on disk; only named agents, never `general-purpose` |
| Learning loop | three layers of memory, each read by the agents that need it: **facts** (this project, `docs/handoff/`, ≤ 100 visible), **best-practices** (this stack, per project), **lessons** (method, every project, `shared/lessons.md`, one line ≤ 300 bytes carrying the `WHERE` of its future rule; `provisional` until a retro promotes it — writing the rule at that `WHERE` and deleting the line in the same PR). Developers log `BUDGET`/`STALL`; the reviewer writes repeated findings as facts; `run-wave` launches `retro` when the board is complete; retro writes all three layers and **merges its own text-only PRs immediately** (audit prefix) so nothing waits on a human — unless `automerge: false` or draft mode. The promotion of a lesson into this repo's own templates is part of that same PR: a rule nobody can read yet is a rule lost |
