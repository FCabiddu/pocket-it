...
| `verify.sh <PR|branch>` | lint, type-check, affected tests on the PR branch in a throwaway worktree, ≤ 40 lines. The reviewer runs it before reading. Exit 0 green, 1 the branch’s own red, 3 a red that also fails at the base commit it pinned for the run (re-run there, once, only for the checks that failed, and only when it could establish the base can run them at all), 2 could not run — including an installed dependency tree that disagrees with the lockfile, which is refused before any check runs (a verdict about code that is not installed is not a verdict) |
| `install-drift.sh check|reinstall [dir]` | one question each way: `check` compares the installed `node_modules` with the lockfile (npm formats v1/v2/v3; any other package manager says "drift check not supported", never a silent pass) — exit 0 match, 1 drift, 3 no verdict; `reinstall` runs the lockfile's own frozen install (`npm ci`, `pnpm|yarn|bun` equivalents) in that checkout, but only when nothing is running against it — a worktree in use or `--running <what>` defers it to `handoff.sh log` and exits 4. `verify.sh` calls `check`; the closing step of `/quickfix` and `/run-wave` calls `reinstall` after merging a PR that touched a lockfile. Test: `install-drift.test.sh` |
