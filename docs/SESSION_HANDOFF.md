...
- A pixel check for clipped text must neutralise the whole class of clippers, not an example list: every overflow value (hidden/clip/auto+fixed height, -webkit-line-clamp), clip-path and clip: rect(), contain: paint|content|strict, content-visibility, mask/mask-image/-webkit-mask-*/mask-border, overflow-clip-margin — measured: with contain:paint or mask-image alone, the as-is and overflow:visible+clip-path:none shots are identical while the ink is still cut, so a partial neutralisation declares a clipped text clean.
