...
- guard.sh: 'git push > file', 'git -c k=v push' and 'env X=1 git push' (and the other assignment/refspec fail-opens) are CLOSED by PI-13 (#47) — the classifier now tokenises the raw command quote-aware and classifies after stripping leading assignments/globals. Kept as history.
